...
export const findSessionUser = async (env, payload, nowSec) => {
  ...
  return env.DB.prepare(`SELECT u.id, u.login, u.name, u.avatar_url FROM web_sessions s JOIN users u ON u.id = s.user_id
...
export const revokeSession = async (env, value, nowSec) => {
  ...
  if (!isDeviceTokenFormat(payload?.sid)) return;
