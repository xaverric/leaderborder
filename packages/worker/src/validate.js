...
export const validateRow = (row, now) => {
  ...
  if (typeof row.costUsd !== "number" || !Number.isFinite(row.costUsd) || row.costUsd < 0) return "costUsd: must be a finite number >= 0";
  return null;
};
...
export const validateDeviceBody = (body) => {
  if (!isObject(body)) return "body: must be an object";
  if (typeof body.githubToken !== "string" || body.githubToken.length < 1 || body.githubToken.length > 255) return "githubToken: required";
  if (!isUuidV4(body.deviceId)) return "deviceId: must be a uuid v4";
  if (typeof body.deviceName !== "string" || body.deviceName.trim().length < 1 || body.deviceName.length > 60) return "deviceName: must be 1-60 chars";
  return null;
};
